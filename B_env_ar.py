''' Only works on Ray 2.7 or later as they had a bug on the MultiDiscrete
    ...
    Autorregresive model with 2 hierarchical RL policies.
...
from gymnasium.spaces import Box, MultiDiscrete, Dict, Discrete, Tuple
...
class PuzzleEnvironment:
    ...
    def _high_level_step(self, action):
        ...
        # TODO: this one is a good idea but it gives 'out of bound' error in some cases need to review why.
...
class PuzzleGymEnv(MultiAgentEnv):
    ...
    def __init__(self, config=None):
        ...
        self.high_level_action_space = Tuple(Discrete(self.env.num_pieces),
                                            Discrete(self.env.num_sides))    # active_piece_id,  # side_index. The index of the side (not the actual labeled value).

        #  Low level - select active piece and side to connect to (side mask depends on the target piece)
        self.low_level_action_space = Tuple(Discrete(self.env.num_pieces),
                                            Discrete(self.env.num_sides))    # active_piece_id,  # side_index. The index of the side (not the actual labeled value).
        ...
        self.high_level_obs_space = Dict({
                   ...
                   'available_pieces_sides' : Box(low=-1, high=100, shape=(self.env.num_pieces), dtype=np.int8),    # Availability of pieces and sides. Columns are the side values, and the last column indicates the availability of the piece
                   'available_connections'  : Box(low=-1, high=1, shape=(self.env.num_pieces  ,), dtype=np.int8),           # Availability of connections
