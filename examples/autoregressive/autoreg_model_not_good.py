...
''' ESTA SOLUCION NO ME CONVENCE porque esto esta mal:
 a1_input = torch.unsqueeze(a1_logits.argmax(dim=-1).float(), 1)  # Sample or take argmax from a1_logits
    Esta parte tiene que ser un sample de la distribucion de a1, no el argmax de los logits.
    Y tiene que venir de la distribucion de a1, no del modelo!

    ADEMAS Da un error de matmult.
'''
...
class TorchAutoregressiveActionModel(TorchModelV2, nn.Module):
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        # NOTE: this is an example, but this should come from the distribution.
        a1_input = torch.unsqueeze(a1_logits.argmax(dim=-1).float(), 1)  # Sample or take argmax from a1_logits
