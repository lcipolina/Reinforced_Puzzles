...
# Save console output to a file: python3 env_hier.py > rllib_output.log 2>&1
...
class HierarchicalWindyMazeEnv(MultiAgentEnv):
    ...
    def __init__(self, env_config):
        super().__init__()
        # Bring the env for the step of the low level agent
        self.flat_env = WindyMazeEnv(env_config) # Flat environment -  meaning the environment without the hierarchical structure

    #----------------------------------------------
    # reset
    #----------------------------------------------
    def reset(self, *, seed=None, options=None):
        '''Starts a new episode. Returns observations for the first agent.'''
        ...
        # Build Obs for next agent
        # Chooses first agent to be the high level agent
        return {
    ...
    def step(self, action_dict):
        '''
        Turn-based game: each action is passed as the observation for the next agent.
        Each agent has to see the previou's agents actions to take their action.
        .. therefore we need to step through *one* agent at a time
        Only agents receiving observations take actions and receive rewards
        Swaps between high_level/low_level rounds
        '''
    ...
    def _high_level_step(self, action):
        ...
        # Prepares observation for the next agent
        # Selects next agent
        self.low_level_agent_id = "low_level_{}".format(self.num_high_level_steps)
        obs = {self.low_level_agent_id: [self.cur_obs, self.current_goal]}
        rew = {self.low_level_agent_id: 0} # rew for the low level agent - for me this is wrong - this should be the rew for the high level!
